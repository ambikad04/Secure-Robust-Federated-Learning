...
def process_sparse_grad(grads):
    ...
    for i in range(1, len(grads)):
        client_grads = np.append(client_grads, grads[i].flatten().numpy()) # output a flattened array
    return client_grads
...
def process_grad(grads):
    ...
    for i in range(1, len(grads)):
        client_grads = np.append(client_grads, grads[i].numpy()) # output a flattened array
